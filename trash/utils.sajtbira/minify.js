module.exports = async function () {
  ...
  let view_files = [];
  file.walkSync(
    original_views_directory,
    function (current_directory, directories, files) {
  ...
  for (let view_file of view_files) {
    ...
    if (!fs_sync.existsSync(path.dirname(minified_path))) {
      await fs.mkdir(path.dirname(minified_path));
    }

    const new_name = path.format({
      ...path.parse(view_file),
      base: "",
      ext: ".html",
    });
    ...
    await fs.writeFile(
      path.join(minified_views_directory, relative_path),
      minified
    );
  }
  ...
  for (let css_file of css_files) {
    ...
    await fs.writeFile(
      path.join(minified_css_directory, relative_path),
      minified
    );
  }
  ...
  for (let js_file of js_files) {
    ...
    await fs.writeFile(
      path.join(minified_js_directory, relative_path),
      minified
    );
  }
