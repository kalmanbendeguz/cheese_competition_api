Product_Schema.pre('validate', async function () {
    ...
    if (!this.competition_id) { // nem így kéne ellenőrizni, hanem hogy jelen van-e a field. (?)
        this.competition_id = (await mongoose.model('Competition').findOne(
            { archived: false },
            { _id: 1 }
        )).id
    }
    if (!this.public_id) { // nem így kéne ellenőrizni, hanem hogy jelen van-e a field. (?)
    ...
    if (!this.secret_id) { // nem így kéne ellenőrizni, hanem hogy jelen van-e a field. (?)
        const randomstring = require('randomstring')
        const forbidden_id_parts = require('../static/forbidden_id_parts')
...
Product_Schema.pre('save', async () => { // ezt jobban átnézni, hogy milyen opciók vannak hozzá!
    console.log(`pre('save')`)
})

Product_Schema.pre('remove', { document: true },
    async function (next) {
