const send_restore_password_email = async function (req, res, next) {
    try {
        ...
        const html = `
            ${req.app.locals.dict[res.locals.lang].restore_password_please_click_on_link} 
            ${process.env.SERVER_URL}/restore_password?restore_id=${res.locals.restore_link_identifier}
            <br>
