const check_restore_link_valid = async function (req, res, next) {
  try {
    ...
    const active_password_reset =
      await req.app.models.judge_active_password_reset.exists({
        restore_id: req.query.restore_id,
      });
    ...
    req.app.set_session_context(
      req.session,
      "errors",
      req.app.locals.dict[res.locals.lang].restore_password_link_invalid
    );
