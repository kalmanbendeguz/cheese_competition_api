const check_user_exists = async function (req, res, next) {
    try {
        ...
        res.locals.user = await req.app.models.judge_user.findOne({ email: res.locals.active_password_reset.email })
        ...
        req.app.set_session_context(
            req.session,
            'errors',
            req.app.locals.dict[res.locals.lang].restore_password_link_user_does_not_exist
        )
