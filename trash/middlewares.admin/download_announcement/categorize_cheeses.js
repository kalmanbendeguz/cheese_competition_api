const categorize_cheeses = function() {
    ...
    return function(req, res, next) {
        ...
        for(let cheese of res.locals.cheeses){
            cheese.category_string = create_category_string(cheese.product_category_list)
            cheese.qualification_string = get_qualification_string_by_score(cheese.average_score)
        }

        res.locals.cheeses.sort((a, b) => a.category_string.localeCompare(b.category_string))

        let category_strings_with_duplicates = res.locals.cheeses.map(a => a.category_string)

        res.locals.category_strings = [...new Set(category_strings_with_duplicates)]

        res.locals.results_by_categories = res.locals.category_strings.map(category_string => {
            let category_object = {}
            category_object.category_array = create_category_array(category_string)
            category_object.category_translated = translate_category_array(category_object.category_array)
            category_object.cheeses = res.locals.cheeses.filter(cheese => cheese.category_string === category_string)
            category_object.cheeses.sort((a, b) => a.average_score - b.average_score)
