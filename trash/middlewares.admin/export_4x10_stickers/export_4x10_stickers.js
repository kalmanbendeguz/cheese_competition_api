const export_4x10_stickers = function () {
  return async function (req, res, next) {
    ...
    for (let i = 0; i < cheeses.length; i += 10) {
      ...
      if (chunk.length !== 10) {
        const empty_cheeses = [...Array(10 - chunk.length).keys()].map(
          (i) => empty_cheese
        );
    ...
    cheeses_by_10 = cheeses_by_10.map((_10_cheeses, j) => {
      let current_10_cheeses = _10_cheeses.map((cheese, i) => {
        ...
        if (j === cheeses_by_10.length - 1 && i >= last_page_rows) {
          ...
          returned[
            `${i + 1}x1`
          ] = `${cheese.factory_name} - ${cheese.product_name}\n(${cheese.public_id})`;
        }
    ...
    const content = await fs.readFile("src/static/4x10_matrica.docx", "binary");
