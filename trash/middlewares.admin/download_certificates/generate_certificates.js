const generate_certificates = function() {
    ...
    return async function(req, res, next) {
        ...
        let certificate_zip_file_path = tempy.temporaryFile({name: `oklevelek.zip`});

        const content = fs.readFileSync(
            'src/static/oklevel.docx',
            "binary"
        );
        ...
        for await (let cheese of res.locals.cheeses){
            ...
            let certificate_file_path = tempy.temporaryFile({name: `oklevel_${cheese.public_id}.docx`});

            fs.writeFileSync(certificate_file_path, buf);

            archive.file(certificate_file_path, {name: `oklevel_${cheese.public_id}.docx`});
        }
