const get_ratings_of_cheeses = function () {
  return async function (req, res, next) {
    ...
    for await (let cheese of res.locals.cheeses) {
      let manufacturer_of_cheese = await User_Model.findOne({
        _id: cheese.manufacturer,
      });
      cheese.manufacturer = manufacturer_of_cheese;

      let ratings_of_cheese = await Rating_Model.find({
      ...
      for await (let rating of cheese.ratings) {
        let judge_of_rating = await Judge_Model.findOne({
          email: rating.judge_email,
        });
