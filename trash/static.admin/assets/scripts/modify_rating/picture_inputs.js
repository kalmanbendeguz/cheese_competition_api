...
const remove_picture_buttons = document.querySelectorAll(
...
for (let remove_picture_button of remove_picture_buttons) {
  remove_picture_button.addEventListener(
    "click",
    on_remove_picture_button_click
  );
}

function on_add_picture_button_click(e) {
  const hidden_picture_input_blocks = document.querySelectorAll(
  ...
  if (hidden_picture_input_blocks.length === 1) {
    ...
    const max_picture_reached_div = document.querySelector(
      "#max_picture_count_reached"
    );
  ...
  const next_picture_input_label = next_picture_input_block.querySelector(
    ".picture_input_label"
  );
...
async function update_image_display(e) {
  ...
  const progress_bar_div = e.target.parentNode.querySelector(".progress_bar");
  const remove_picture_button = e.target.parentNode.querySelector(
    ".remove_picture_button"
  );
  const picture_input_label = e.target.parentNode.querySelector(
    ".picture_input_label"
  );
  ...
  try {
    const new_blob = await imageCompression(picture_input.files[0], {
      ...compress_options,
      onProgress: picture_compress_on_progress(progress_bar_div),
    });
    ...
    const new_file = new File([new_blob], new_blob.name, {
      type: new_blob.type,
    });
...
for (let i = 0; i < preloaded_picture_blocks.length; ++i) {
  const file_input =
    preloaded_picture_blocks[i].querySelector(".picture_input");
  const data_transfer = new DataTransfer();

  const picture_preview_div =
    preloaded_picture_blocks[i].querySelector(".picture_preview");
  const picture_preview_img = picture_preview_div.querySelector("img");
  const picture_url = picture_preview_img.src;
  fetch(picture_url)
    .then((response) => response.blob())
    .then((picture_blob) => {
      const picture_file = new File([picture_blob], `picture_${i + 1}.png`, {
        type: picture_blob.type,
      });
