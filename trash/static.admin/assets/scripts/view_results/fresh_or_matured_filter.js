...
for (let type of Object.keys(maturation_time_types)) {
    maturation_time_types[type] = get_cookie(`view_results_show_${type}`) ?? maturation_time_types[type]
}

const rows_of_view_results_table = document.querySelectorAll('#view_results_table tr')

for (let row of rows_of_view_results_table) {
    for (let [type, value] of Object.entries(maturation_time_types)) {
        if(row.hasAttribute(type)) row.style.display = value === 'true' ? '' : 'none'
    }
}
///

// set checkbox states and add listeners
const filter_checkboxes = document.querySelectorAll('input[maturation_time_type_filter]')

for (let filter_checkbox of filter_checkboxes) {
    filter_checkbox.checked = maturation_time_types[filter_checkbox.value] === 'true'
