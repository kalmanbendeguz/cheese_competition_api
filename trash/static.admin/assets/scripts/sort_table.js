function sort_table(column_index, table) {
    ...
    const first_content_before_sort = rows_to_sort[0].cells[column_index].textContent.trim()

    if (rows_to_sort.every(
        row => row.cells[column_index].textContent.trim() === rows_to_sort[0].cells[column_index].textContent.trim()
    )) return
    ...
    const sorter = function (direction) {
        return function (a, b) {
            ...
            if (sort_type === "numeric") {
                ...
                if(isNaN(a_content) && !isNaN(b_content)) return 1
                if(isNaN(a_content) && isNaN(b_content)) return direction * (a_content.localeCompare(b_content))
    ...
    const first_content_after_sort = rows_to_sort[0].cells[column_index].textContent.trim()
