module.exports = async function (req, res, next) {
    try {
        ...
        if (req.user.role === 'competitor') {
            ...
            if(res.locals.forbidden_projections.some(key => (key in (req.query.projection ?? {})))) return res.sendStatus(403)
        }

        if (req.user.role === 'judge') {
            const required_query = ['secret_id']
            const forbidden_query = ['_id', 'public_id']
            res.locals.forbidden_projections = ['_id', 'manufacturer_id', 'public_id', 'product_name', 'factory_name', 'product_description']
