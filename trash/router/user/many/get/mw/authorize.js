module.exports = function (req, res, next) {
    try {
        ...
        if (req.user.role === 'competitor') {
            ...
            const forbidden_fields = ['_id', 'secret_id']
            res.locals.forbidden_projections = ['_id', 'secret_id']

            if (required_fields.some(key => !(key in req.query))) return res.sendStatus(403)
            if (forbidden_fields.some(key => (key in req.query))) return res.sendStatus(403)
