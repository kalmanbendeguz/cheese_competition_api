    ...
    try {
        ...
        const validator = Joi.object({
            ...
            body: Joi.object({
                ...
                // required
                product_name: Joi.string().trim().min(3).max(25).prefs({ convert: false }).optional(),
                // required
                factory_name: Joi.string().trim().min(3).max(80).prefs({ convert: false }).optional(),
                // required
                maturation_time_type: Joi.string().trim().valid('fresh', 'matured').prefs({ convert: false }).optional(),
                // required
                ...
                // required
                milk_type: Joi.string().trim().min(1).prefs({ convert: false }).optional(), // should validate based on json tree!!!!!
                // required
                ...
                product_description: Joi.string().trim().min(25).max(1000).prefs({ convert: false }).optional(),
                // optional
                approved: Joi.boolean().optional(),
                // optional
