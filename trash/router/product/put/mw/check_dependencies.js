module.exports = async function (req, res, next) {
    try {
        ...
        const unique_old_competition_ids = [...new Set(res.locals.products.map(product => product.competition_id.toString()))]
        ...
        })) !== unique_old_competition_ids.length) return res.sendStatus(403)
        ...
        if ('manufacturer_id' in req.body && !(await User_Model.exists({
        ...
        if(req.body.approval_type !== 'payment' && res.locals.products.some(product => product.approval_type === 'payment')) return res.sendStatus(403)
