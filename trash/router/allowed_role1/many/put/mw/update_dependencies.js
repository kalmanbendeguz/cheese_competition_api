module.exports = async function (req, res, next) {
    try {
        console.log('mw:update_dependencies(product/one/many/mw/update_dependencies)')

        if ('email' in req.body && res.locals.allowed_judges.some(allowed_judge => allowed_judge.email !== req.body.email)) {
            ...
            const remove_promises =
                res.locals.allowed_judges.map(allowed_judge =>
                    axios({
                        url: '/api/a/user/o',
                        method: 'put',
