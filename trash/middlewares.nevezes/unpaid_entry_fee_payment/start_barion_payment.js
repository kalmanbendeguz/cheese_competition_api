const start_barion_payment = async function (req, res, next) {
    try {
        ...
        const total_price = Fraction(res.locals.unpaid_cheeses.length).mul(res.locals.entry_fee_amount).toString()
                    ...
                    Items: [
                        {
                            ...
                            Description: req.app.locals.dict[res.locals.lang].entry_fee_item_description,
                            ...
                            ItemTotal: res.locals.unpaid_cheeses.length * res.locals.entry_fee_amount
                        }
                    ]
                }
            ]
        ...
        const start_payment = async () => barion.startPayment(entry_fee_payment, (err, result) => { return {err, result} })
