const format_billing_information = function (req, res, next) {
    try {
        ...
        if(billing_city_iso_3166_2) res.locals.billing_region_iso_3166_2 = billing_city_iso_3166_2
        ...
        res.locals.street = ''
        while(street_array.length > 0 && res.locals.street.length + 1 + street_array[0].length <= 50){
            const part = street_array.shift()
            res.locals.street += ` ${part}`
        }

        res.locals.street2 = ''
        while(street_array.length > 0 && res.locals.street2.length + 1 + street_array[0].length <= 50){
            const part = street_array.shift()
        ...
        res.locals.street3 = ''
        while(street_array.length > 0 && res.locals.street3.length + 1 + street_array[0].length <= 50){
            const part = street_array.shift()
