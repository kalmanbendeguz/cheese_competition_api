const load_product_tree = function (req, res, next) {
    try {
        ...
        res.locals.milk_types = req.app.locals.product_tree.map(milk_type => {
            ...
            if (typeof wrapper.name === 'undefined') throw new Error(`${wrapper.id}: ${req.app.locals.dict[res.locals.lang].no_field_in_dictionary}`)
            return wrapper
        })

        const select_groups =
            req.app.locals.product_tree.map(milk_type =>
                create_select_group(
                    0,
...
function create_select_group(depth, current_id, list, options, dictionary, category_string) {
    if (!options) options = []
    const space = '&nbsp;'
