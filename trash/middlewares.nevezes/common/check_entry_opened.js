const check_entry_opened = async function (req, res, next) {
    try {
        ...
        const entry_opened = await req.app.models.key_value.findOne({ key: 'entry_opened' })

        if (!entry_opened) throw new Error(req.app.locals.dict[res.locals.lang].entry_opened_document_does_not_exist)
