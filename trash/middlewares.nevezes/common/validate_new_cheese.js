const validate_new_cheese = function (req, res, next) {
  try {
    ...
    if (typeof req.body.product_name === "undefined") {
      problems.push(
        req.app.locals.dict[res.locals.lang].product_name_field_missing
      );
    }
    if (typeof req.body.factory_name === "undefined") {
      problems.push(
        req.app.locals.dict[res.locals.lang].factory_name_field_missing
      );
    }
    if (typeof req.body.maturation_time_type === "undefined") {
      problems.push(
        req.app.locals.dict[res.locals.lang].maturation_time_type_missing
      );
    }
    if (req.body.maturation_time_type === "matured") {
      if (typeof req.body.maturation_time_quantity === "undefined") {
        problems.push(
          req.app.locals.dict[res.locals.lang].maturation_time_quantity_missing
        );
      }
      if (typeof req.body.maturation_time_unit === "undefined") {
        problems.push(
          req.app.locals.dict[res.locals.lang].maturation_time_unit_missing
        );
      }
    }
    ...
    if (typeof req.body.product_category_list === "undefined") {
      problems.push(
        req.app.locals.dict[res.locals.lang].product_category_list_missing
      );
    }
    if (typeof req.body.product_description === "undefined") {
      problems.push(
        req.app.locals.dict[res.locals.lang].product_description_missing
      );
    }
    if (req.body.product_name.length < 3) {
      problems.push(
        req.app.locals.dict[res.locals.lang].product_name_too_short
      );
    }
    ...
    if (req.body.factory_name.length < 3) {
      problems.push(
        req.app.locals.dict[res.locals.lang].factory_name_too_short
      );
    }
    ...
    if (
      !(
        req.body.maturation_time_type === "matured" ||
        req.body.maturation_time_type === "fresh"
      )
    ) {
      problems.push(
        req.app.locals.dict[res.locals.lang].invalid_maturation_time_type
      );
    }
    if (req.body.maturation_time_type === "matured") {
      if (req.body.maturation_time_quantity < 1) {
        problems.push(
          req.app.locals.dict[res.locals.lang]
            .maturation_time_quantity_should_be_positive
        );
      }
      if (
        !(
          req.body.maturation_time_unit === "day" ||
          req.body.maturation_time_unit === "week" ||
          req.body.maturation_time_unit === "month"
        )
      ) {
        problems.push(
          req.app.locals.dict[res.locals.lang].invalid_maturation_time_unit
        );
      }
    }

    let milk_types = req.app.locals.product_tree.map(
      (milk_type) => Object.keys(milk_type)[0]
    );
    ...
    if (req.body.product_category_list.length === 0) {
      problems.push(
        req.app.locals.dict[res.locals.lang].invalid_category_empty
      );
    }
    ...
    if (
      !req.app.locals.product_tree.find((e) => Object.keys(e)[0] === top_level)
    ) {
      problems.push(
        req.app.locals.dict[res.locals.lang].invalid_category_first_node_invalid
      );
    }

    const bottom_level = res.locals.category_array.at(-1);
    const bottom_categories = bottom_level_primitives(
      req.app.locals.product_tree,
      null
    );
    if (!bottom_categories.includes(bottom_level)) {
      problems.push(
        req.app.locals.dict[res.locals.lang].invalid_category_last_node_invalid
      );
    }

    let current_tree_array = req.app.locals.product_tree;
    for (let i = 0; i < res.locals.category_array.length; ++i) {
      let sub_tree = current_tree_array.find(
        (e) =>
      ...
      if (typeof sub_tree === "object") {
        ...
        if (i !== res.locals.category_array.length - 1) {
          problems.push(
            req.app.locals.dict[res.locals.lang]
              .invalid_category_lowest_level_category_before_path_ending
          );
          break;
        }
      } else if (typeof sub_tree === "undefined") {
        problems.push(
          req.app.locals.dict[res.locals.lang].invalid_category_split_in_path
        );
        break;
      }
    }
    ...
    if (req.body.product_description.length < 25) {
      problems.push(
        req.app.locals.dict[res.locals.lang].product_description_too_short
      );
    }

    if (req.body.product_description.length > 1000) {
      problems.push(
        req.app.locals.dict[res.locals.lang].product_description_too_long
      );
    }
