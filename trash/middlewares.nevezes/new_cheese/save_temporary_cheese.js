const save_temporary_cheese = async function (req, res, next) {
  try {
    ...
    do {
      ...
      existing_cheese =
        (await req.app.models.cheese.exists({ public_id: public_id })) ||
        (await req.app.models.cheese.exists({ secret_id: public_id })) ||
        (await req.app.models.temporary_cheese.exists({
          "cheese.public_id": public_id,
        })) ||
        (await req.app.models.temporary_cheese.exists({
          "cheese.secret_id": public_id,
        })) ||
        (await req.app.models.archived_cheese.exists({
          public_id: public_id,
        })) ||
        (await req.app.models.archived_cheese.exists({
          secret_id: public_id,
        })) ||
        (await req.app.models.unpaid_cheese.exists({
          "product.public_id": public_id,
        })) ||
        (await req.app.models.unpaid_cheese.exists({
          "product.secret_id": public_id,
        }));
    ...
    do {
      ...
      existing_cheese =
        (await req.app.models.cheese.exists({ secret_id: secret_id })) ||
        (await req.app.models.cheese.exists({ public_id: secret_id })) ||
        (await req.app.models.temporary_cheese.exists({
          "cheese.secret_id": secret_id,
        })) ||
        (await req.app.models.temporary_cheese.exists({
          "cheese.public_id": secret_id,
        })) ||
        (await req.app.models.archived_cheese.exists({
          secret_id: secret_id,
        })) ||
    ...
    const cheese = new req.app.models.cheese(res.locals.cheese);
