const check_payment_succeeded = async function (req, res, next) {
    try {
        ...
        const get_payment_state = async () => 
            barion.getPaymentState({PaymentId: req.body.PaymentId}, (err, data) => { return {err, data} })
        ...
        if(state.data.Status === 'Canceled' || state.data.Status === 'Expired'){
            ...
            await req.app.models.pending_payment.findByIdAndDelete(pending_payment._id)
        }
