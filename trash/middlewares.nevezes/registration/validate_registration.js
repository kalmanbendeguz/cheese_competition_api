const validate_registration = function (req, res, next) {
  try {
    ...
    if (req.body.contact_phone_number.length === 0) {
      problems.push(
        req.app.locals.dict[res.locals.lang].contact_phone_number_empty
      );
    }
    ...
    if (req.body.billing_street_type.length === 0) {
      problems.push(
        req.app.locals.dict[res.locals.lang].billing_street_type_empty
      );
    }
    if (req.body.billing_house_number.length === 0) {
      problems.push(
        req.app.locals.dict[res.locals.lang].billing_house_number_empty
      );
    }
    ...
    if (req.body.general_terms_and_condition_check !== "approved") {
      problems.push(
        req.app.locals.dict[res.locals.lang]
          .general_terms_and_condition_not_approved
      );
    }
