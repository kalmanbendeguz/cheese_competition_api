const send_verification_email = async function (req, res, next) {
  try {
    ...
    const title =
      req.app.locals.dict[res.locals.lang].confirm_registration_email_title;
    const html = `
            ${
              req.app.locals.dict[res.locals.lang]
                .confirm_registration_please_click_on_link
            }
            ${process.env.SERVER_URL}/confirm_registration?confirm_id=${
      res.locals.confirm_id
    }
            <br>
            ${
              req.app.locals.dict[res.locals.lang]
