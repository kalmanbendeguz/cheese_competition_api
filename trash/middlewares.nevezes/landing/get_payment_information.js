const get_payment_information = async function (req, res, next) {
  try {
    ...
    res.locals.paid_competition = (
      await req.app.models.key_value.findOne({ key: "paid_competition" })
    )?.value;

    const Fraction = require("fraction.js");
    res.locals.entry_fee_total = Fraction(res.locals.unpaid_cheeses.length)
