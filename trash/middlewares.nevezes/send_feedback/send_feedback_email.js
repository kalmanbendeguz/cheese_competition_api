const send_feedback_email = async function (req, res, next) {
  try {
    ...
    const title = `${process.env.APPLICATION_NAME} ${
      req.app.locals.dict[res.locals.lang].feedback
    } (${req.app.locals.dict[res.locals.lang].sender}: ${
      req?.user?.email ??
      req.app.locals.dict[res.locals.lang].feedback_email_sender_not_logged_in
    })`;
