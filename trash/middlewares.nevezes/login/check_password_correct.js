const check_password_correct = async function (req, res, next) {
    try {
        ...
        const is_password_valid = await bcrypt.compare(req.body.password, res.locals.user.hashed_password)
        ...
        req.app.set_session_context(req.session, 'errors', req.app.locals.dict[res.locals.lang].wrong_password)
