...
module.exports = async (body, user) => {
    ...
    const authorizer_results = body.map((entry_fee_payment) =>
        authorizer(entry_fee_payment, 'create', user)
    )
    ...
    for (const entry_fee_payment of body) {
        ...
        do {
            ...
            existing_entry_fee_payment =
                ...
                body.some(
                    (entry_fee_payment) =>
                        entry_fee_payment.pos_transaction_id ===
                        pos_transaction_id
                )
        ...
        do {
            ...
            existing_entry_fee_payment =
                ...
                body.some(
                    (entry_fee_payment) =>
                        entry_fee_payment.confirm_payment_id ===
                        confirm_payment_id
                )
        } while (existing_entry_fee_payment)
    ...
    // 6. create
    const entry_fee_payments = _entry_fee_payments.map(
        (entry_fee_payment) => new Entry_Fee_Payment_Model(entry_fee_payment)
    )
    ...
    // 9. save
    const saver_promises = entry_fee_payments.map((entry_fee_payment) =>
