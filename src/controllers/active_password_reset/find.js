...
module.exports = async (query, user) => {
    ...
    try { await find_active_password_reset_validator.validateAsync(query) }
    ...
    if (!filter_authorizer_result.authorized) { return { code: 403, data: filter_authorizer_result.message } }

    // 3. authorize {query.projection, user}
    const projection_authorizer_result = authorizer(query.projection, 'project', user)
    if (!projection_authorizer_result.authorized) { return { code: 403, data: projection_authorizer_result.message } }
