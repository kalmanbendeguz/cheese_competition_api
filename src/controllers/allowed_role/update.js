...
module.exports = async (data, user) => {
    ...
    try { await update_allowed_role_validator.validateAsync(data) }
    ...
    if (user.role === 'organizer' && (
        (!Array.isArray(data.body.allowed_roles) && data.body.allowed_roles.split(' ').includes('-organizer'))
        || (Array.isArray(data.body.allowed_roles) && !data.body.includes('organizer'))
    )) return {
        code: 403,
    ...
    let roles_to_add = []
    let roles_to_remove = []
    if (Array.isArray(data.body.allowed_roles)) { // it is an array
        roles_to_add = data.body.allowed_roles
        roles_to_remove = all_roles.filter(role => !roles_to_add.includes(role))
    } else { // it is a string
    ...
    for (const allowed_role of allowed_roles) {
        ...
        const modified_allowed_roles_array = [...new Set(current_allowed_roles_array.filter(ar => !roles_to_remove.includes(ar)).concat(roles_to_add))]
    ...
    try {
        const validator_promises = allowed_roles.map(allowed_role => allowed_role_validator.validateAsync(allowed_role))
    ...
    const update_user_promises = allowed_roles.map(allowed_role => update_user(
        {
            ...
            body: { roles: roles_to_remove.map(ar => `-${ar}`).join(' ') }
        },
        {
            role: 'SERVER'
        }
    ...
    // 10. save
    const saver_promises = allowed_roles.map(allowed_role => allowed_role.save())
