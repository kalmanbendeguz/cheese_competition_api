...
module.exports = async (body, user) => {
    ...
    try { await create_allowed_role_validator.validateAsync(body) }
    ...
    const authorizer_results = body.map(allowed_role => authorizer(allowed_role, 'create', user))
    ...
    for (const allowed_role of body) {
        ...
        if (await Allowed_Role_Model.exists({ email: allowed_role.email })) return {
            code: 409,
