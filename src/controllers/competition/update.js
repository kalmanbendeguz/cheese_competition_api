...
module.exports = async (data, user) => {
    ...
    try { await update_competition_validator.validateAsync(data) }
    ...
    if (!update_authorizer_result.authorized) { return { code: 403, data: update_authorizer_result.message } }
    ...
    for (const competition of competitions) {
        ...
        if (typeof current_update.archived !== 'undefined' && !competition.archived && current_update.archived) {
        ...
        if (typeof current_update.entry_opened !== 'undefined') {
            if (!competition.entry_opened && current_update.entry_opened) current_update.last_entry_open_date = now
            if (competition.entry_opened && !current_update.entry_opened) current_update.last_entry_close_date = now
        }
        if (typeof current_update.competition_opened !== 'undefined') {
            if (!competition.competition_opened && current_update.competition_opened) current_update.last_competition_open_date = now
            if (competition.competition_opened && !current_update.competition_opened) current_update.last_competition_close_date = now
        }
        if (typeof current_update.payment_needed !== 'undefined' && competition.payment_needed && !current_update.payment_needed) {
            current_remove = current_remove.concat(['entry_fee_amount', 'entry_fee_currency'])
        }
